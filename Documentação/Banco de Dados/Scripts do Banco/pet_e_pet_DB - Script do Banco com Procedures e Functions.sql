...
DELIMITER $$
CREATE DEFINER=`root`@`localhost` PROCEDURE `procedure_cadastrar_cliente` (IN `p_nome` VARCHAR(100), IN `p_cpf` VARCHAR(14), IN `p_data_nascimento` DATE, IN `p_telefone` VARCHAR(15), IN `p_email` VARCHAR(100), IN `p_cep` VARCHAR(10), IN `p_rua` VARCHAR(150), IN `p_bairro` VARCHAR(100), IN `p_numero` VARCHAR(20), IN `p_complemento` VARCHAR(100), IN `p_estado` VARCHAR(2), IN `p_observacoes` TEXT)   BEGIN
    INSERT INTO `cliente` (
        `nome`,
        `cpf`,
        `data_nascimento`,
    ...
    SELECT LAST_INSERT_ID() AS id_cliente_cadastrado;
END$$
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_CadastrarCliente` (IN `p_nome` VARCHAR(100), IN `p_cpf` VARCHAR(14), IN `p_data_nascimento` DATE, IN `p_telefone` VARCHAR(15), IN `p_email` VARCHAR(100), IN `p_cep` VARCHAR(10), IN `p_rua` VARCHAR(150), IN `p_bairro` VARCHAR(100), IN `p_numero` VARCHAR(20), IN `p_complemento` VARCHAR(100), IN `p_observacoes` TEXT)   BEGIN
    INSERT INTO `cliente` (
        `nome`, `cpf`, `data_nascimento`, `telefone`, `email`, `cep`,
        `rua`, `bairro`, `numero`, `complemento`, `observacoes`
    ) VALUES (
        p_nome, p_cpf, p_data_nascimento, p_telefone, p_email, p_cep,
        p_rua, p_bairro, p_numero, p_complemento, p_observacoes
    );
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_CadastrarFornecedor` (IN `p_nome_fantasia` VARCHAR(100), IN `p_razao_social` VARCHAR(150), IN `p_cnpj` VARCHAR(18), IN `p_telefone` VARCHAR(15), IN `p_email` VARCHAR(100), IN `p_contato` VARCHAR(100), IN `p_cep` VARCHAR(10), IN `p_rua` VARCHAR(150), IN `p_bairro` VARCHAR(100), IN `p_numero` VARCHAR(20), IN `p_complemento` VARCHAR(100), IN `p_observacoes` TEXT)   BEGIN
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_CadastrarPet` (IN `p_nome` VARCHAR(50), IN `p_cliente_id` INT, IN `p_especie_id` INT, IN `p_raca_id` INT, IN `p_data_nascimento` DATE, IN `p_peso` DECIMAL(5,2), IN `p_cor` VARCHAR(30), IN `p_castrado` TINYINT(1), IN `p_vacinado` TINYINT(1), IN `p_observacoes` TEXT, IN `p_foto` VARCHAR(255), IN `p_porte` ENUM('Pequeno','Medio','Grande'))   BEGIN
    INSERT INTO `pet` (
        `nome`, `cliente_id`, `especie_id`, `raca_id`, `data_nascimento`,
        `peso`, `cor`, `castrado`, `vacinado`, `observacoes`, `foto`, `porte`
    ) VALUES (
        p_nome, p_cliente_id, p_especie_id, p_raca_id, p_data_nascimento,
        p_peso, p_cor, p_castrado, p_vacinado, p_observacoes, p_foto, p_porte
    );
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_CadastrarProduto` (IN `p_nome` VARCHAR(100), IN `p_descricao` TEXT, IN `p_categoria_id` INT, IN `p_fornecedor_padrao_id` INT, IN `p_preco_custo` DECIMAL(10,2), IN `p_preco_venda` DECIMAL(10,2), IN `p_quantidade_estoque` INT, IN `p_estoque_minimo` INT, IN `p_codigo_barras` VARCHAR(50))   BEGIN
    INSERT INTO `produto` (
        `nome`, `descricao`, `categoria_id`, `fornecedor_padrao_id`,
        `preco_custo`, `preco_venda`, `quantidade_estoque`, `estoque_minimo`, `codigo_barras`
    ) VALUES (
        p_nome, p_descricao, p_categoria_id, p_fornecedor_padrao_id,
        p_preco_custo, p_preco_venda, p_quantidade_estoque, p_estoque_minimo, p_codigo_barras
    );
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `sp_cadastrar_cliente` (IN `p_nome` VARCHAR(100), IN `p_cpf` VARCHAR(14), IN `p_data_nascimento` DATE, IN `p_telefone` VARCHAR(15), IN `p_email` VARCHAR(100), IN `p_cep` VARCHAR(10), IN `p_rua` VARCHAR(150), IN `p_bairro` VARCHAR(100), IN `p_numero` VARCHAR(20), IN `p_complemento` VARCHAR(100), IN `p_estado` VARCHAR(2), IN `p_observacoes` TEXT)   BEGIN
    INSERT INTO `cliente` (
        `nome`,
        `cpf`,
        `data_nascimento`,
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `SP_RegistrarVacina` (IN `p_pet_id` INT, IN `p_nome_vacina` VARCHAR(100), IN `p_data_aplicacao` DATE, IN `p_data_proxima` DATE, IN `p_veterinario` VARCHAR(100), IN `p_observacoes` TEXT)   BEGIN
    ...
    -- Opcional: Atualizar o indicador 'vacinado' na tabela PET
    UPDATE `pet`
    SET `vacinado` = 1
    WHERE `id` = p_pet_id;
END$$
...
CREATE DEFINER=`root`@`localhost` FUNCTION `proxima_vacina_pet` (`p_pet_id` INT) RETURNS DATE READS SQL DATA BEGIN
    ...
    AND data_proxima >= CURDATE(); -- Apenas datas futuras ou hoje
...
CREATE TABLE agendamento (
  ...
  total_estimado decimal(10,2) NOT NULL DEFAULT 0.00 COMMENT 'Valor estimado do serviço no momento do agendamento.'
...
CREATE TABLE cliente (
  ...
  created_at timestamp NOT NULL DEFAULT current_timestamp(),
  updated_at timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  estado varchar(20) DEFAULT NULL COMMENT 'Estado'
